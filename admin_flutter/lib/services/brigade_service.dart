...
class BrigadeService {
  ...
  Future<List<dynamic>> getAllBrigades() async {
    try {
      final headers = await _getHeaders();
      final response = await http.get(
        Uri.parse('$baseUrl/brigade/all'),
        headers: headers,
      );
      ...
      if (response.statusCode == 200) {
        ...
        throw Exception('Ошибка загрузки бригад');
      }
    } catch (e) {
  ...
  Future<List<dynamic>> getBrigadeMasters(int brigadeId) async {
    try {
      final headers = await _getHeaders();
      final response = await http.get(
        Uri.parse('$baseUrl/orders/brigade/$brigadeId/masters'),
        headers: headers,
      );
      ...
      if (response.statusCode == 200) {
        ...
        throw Exception('Ошибка загрузки мастеров бригады');
      }
    } catch (e) {
      throw Exception(e.toString());
    }
  ...
  Future<List<dynamic>> getAllMasters() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        throw Exception('Ошибка загрузки мастеров');
      }
    } catch (e) {
      throw Exception(e.toString());
    }
  ...
  Future<void> addMasterToBrigade(int brigadeId, int userId) async {
    try {
      ...
      final response = await http.post(
        Uri.parse('$baseUrl/brigade/$brigadeId/add-master'),
        headers: headers,
        body: jsonEncode({'userId': userId}),
      );
      
      if (response.statusCode != 200) {
        throw Exception('Ошибка добавления мастера в бригаду');
      }
    } catch (e) {
      throw Exception(e.toString());
    }
  ...
  Future<void> removeMasterFromBrigade(int brigadeId, int userId) async {
    try {
      final headers = await _getHeaders();
      final response = await http.post(
        Uri.parse('$baseUrl/brigade/$brigadeId/remove-master'),
        headers: headers,
        body: jsonEncode({'userId': userId}),
      );
      
      if (response.statusCode != 200) {
        throw Exception('Ошибка удаления мастера из бригады');
      }
    } catch (e) {
      throw Exception(e.toString());
    }
